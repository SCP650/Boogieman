...
// ===== OLD FLANKER SCRIPT - EXAMPLE FOR READING, FORMATTING, AND SAVING DATA (but we're now using DataSavingBoogie) =====



public enum InterventionType {Intervention, Showcase};

public static class DataSavingFlanker {
	...
	static string[] InfoHeader = {"ID", 	"Test Group"};

  static string[] DataHeader = {"Trial Date","Baseline HR", "Baseline SO2", "Incongruent Accuracy (total correct trials)", "Incongruent Accuracy %",
                  "Congruent Accuracy (total correct trials)", "Congruent Accuracy %", "Total Correct", "Total Correct %",
                  "Total Trials", "Total Incorrect", "Total Misses (out of time)", "Total Failed Trials", "Conflict Resolution Accuracy",  "Conflict Resolution Reaction Time", "Average Incongruent Reaction Time", "Avergage Congruent Reaction Time", "Average Reaction Time",
                                  ...
                                  "Highest Level Completed", "Total Time", "Heart Rate", "SO2", "Smilometer", "Would you play this again?", "Positive Affect", "Negative Affect"};
  ...
  // Other variables
  public static InterventionType CurrentInterventionType = InterventionType.Intervention;


  // Call this to save trial data into the .csv file
  public static void SaveData(string ID, string[] newData) {
		if (CurrentInterventionType == InterventionType.Showcase) {
			return;
		}
